...
/* ---------- Customer Dashboard ---------- */
function CustomerDashboard({ profile, quotations, orders, favProducts, respondQuotation, setRoute }) {
  ...
  const tabs = [["overview","Overview"],["orders","My Orders"],["quotations","My Quotations"],["favourites","Favourite Products"],["profile","My Profile"]];
  return (
      ...
      <div>
        ...
        {tab === "profile" && <>
          ...
          <div className="grid sm:grid-cols-2 gap-4 max-w-lg">
            <Field label="Company Name"><input className="in" defaultValue={profile?.company_name} disabled /></Field>
            <Field label="Phone"><input className="in" defaultValue={profile?.phone} disabled /></Field>
...
function AdminDashboard({ profile, quotations, orders, products, customers, refreshAll, updateQuotation, updateOrder, adjustStock, addProduct, removeProduct }) {
  const [tab, setTab] = useState("overview");
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-8 grid md:grid-cols-[220px_1fr] gap-8">
      ...
      <div>
        ...
        {tab === "settings" && profile?.role === "admin" && <div><h1 className="font-black text-2xl text-[#14181C] mb-6">Company Settings</h1><p className="text-sm text-[#3F4A52]">Company name, phone and address are managed in the `company_settings` table.</p></div>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (s) {
        ...
        else if (p?.role === "staff" || p?.role === "admin") { await fetchQuotations(); await fetchOrders(); await fetchCustomers(); }
      }
      setLoaded(true);
    })();
  ...
  const pages = {
    ...
    "customer-dashboard": <CustomerDashboard profile={profile} quotations={quotations} orders={orders} favProducts={favProducts} respondQuotation={respondQuotation} setRoute={setRoute} />,
    "admin-dashboard": <AdminDashboard profile={profile} quotations={quotations} orders={orders} products={products} customers={customers} updateQuotation={updateQuotation} updateOrder={updateOrder} adjustStock={adjustStock} addProduct={addProduct} removeProduct={removeProduct} />,
  };
